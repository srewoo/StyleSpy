...
const COLUMNS: Column[] = [
  { key: 'tag', label: 'Tag', width: 70, get: (s) => s.identity.tag, cell: (s) => h('span', { class: 'tag-pill', text: s.identity.tag }) },
  { key: 'text', label: 'Text', width: 220, get: (s) => s.text },
  { key: 'color', label: 'Color', width: 120, get: (s) => toHex(s.styles.color), cell: (s) => colorCell(s.styles.color) },
  { key: 'bg', label: 'BG', width: 120, get: (s) => toHex(s.styles.backgroundColor), cell: (s) => colorCell(s.styles.backgroundColor) },
  { key: 'fontFamily', label: 'Font Family', width: 130, get: (s) => s.styles.fontFamily.split(',')[0] ?? '' },
  { key: 'fontSize', label: 'Size', width: 64, get: (s) => s.styles.fontSize },
  { key: 'fontWeight', label: 'Weight', width: 70, get: (s) => s.styles.fontWeight },
  { key: 'lineHeight', label: 'Line H', width: 70, get: (s) => s.styles.lineHeight },
  { key: 'letterSpacing', label: 'Letter', width: 72, get: (s) => s.styles.letterSpacing },
  { key: 'textAlign', label: 'Align', width: 70, get: (s) => s.styles.textAlign },
  { key: 'visibility', label: 'Visibility', width: 110, get: (s) => s.visibility, cell: (s) => visCell(s) },
  { key: 'locator', label: 'Locator', width: 96, get: (s) => classifyLocator(s.identity), cell: (s) => locatorCell(s) },
  { key: 'cssSelector', label: 'CSS Selector', width: 240, get: (s) => s.identity.cssSelector, cell: (s) => h('code', { text: s.identity.cssSelector }) },
  { key: 'xpath', label: 'XPath', width: 200, get: (s) => s.identity.xpath, cell: (s) => h('code', { class: 'faint', text: s.identity.xpath }) },
  { key: 'id', label: 'ID', width: 110, get: (s) => s.identity.id ?? '' },
  { key: 'classes', label: 'Classes', width: 180, get: (s) => s.identity.classNames.join(' ') },
];
...
function colorCell(color: string): Node {
  return h('span', { class: 'cc' }, swatch(color), h('code', { text: toHex(color) }));
}
function visCell(s: ElementSnapshot): Node {
  const cls = s.visibility === 'visible' ? 'vis-ok' : s.state !== 'base' ? 'vis-dyn' : 'vis-hidden';
  return h('span', { class: cls, text: s.state !== 'base' ? `${s.visibility}·${s.state}` : s.visibility });
}
...
function rows(): ElementSnapshot[] {
  ...
  return [...filtered].sort((a, b) => col.get(a).localeCompare(col.get(b)) * state.sortDir);
}

function toolbar(): HTMLElement {
  return h('header', { class: 'tbar' },
    h('img', { class: 'logo', attrs: { src: '/icons/icon48.png', alt: '' } }),
    h('span', { class: 'title', text: 'StyleSpy' }),
    h('span', { class: 'sub', text: `· ${state.snapshots.length} elements · ${state.url}`, title: state.url }),
    h('input', { class: 'search', attrs: { type: 'search', placeholder: 'Search…', value: state.query }, on: { input: (e) => { state.query = (e.target as HTMLInputElement).value; render(); } } }),
    h('button', { class: 'btn', text: 'CSV', on: { click: () => downloadText('stylespy.csv', toCsv(rows()), 'text/csv') } }),
    h('button', { class: 'btn', text: 'JSON', on: { click: () => downloadText('stylespy.json', toJson(rows()), 'application/json') } }),
    h('button', { class: 'btn btn--primary', text: 'Copy', on: { click: () => void copyText(toCsv(rows())) } }),
  );
}

function columnToggles(): HTMLElement {
  return h('div', { class: 'col-toggles' },
    ...COLUMNS.map((c) => {
...
function spacer(height: number): HTMLElement {
  const td = h('td', { attrs: { colspan: String(visibleCols.length) }, style: { height: `${height}px`, padding: '0', border: 'none' } });
...
function render(): void {
  ...
  if (state.snapshots.length === 0) {
    root.append(
      toolbar(),
      h('div', { class: 'empty', text: 'No capture found. Open the side panel, click “Capture Page”, then “Full table”.' }),
    );
    return;
  }
...
async function load(): Promise<void> {
  ...
  const cap = data[CAPTURE_STORAGE_KEY] as { snapshots: ElementSnapshot[]; url: string } | undefined;
  if (cap) {
