...
function entryRow(e: MutationEntry): HTMLElement {
  return h('div', { class: `mut ${KIND_CLASS[e.kind]}` },
...
export function renderMutations(): HTMLElement {
  ...
  if (mutations.length === 0) {
    feed.append(h('div', { class: 'empty', text: mutationLogOn ? 'Listening… interact with the page to see changes.' : 'Start logging to watch elements get added, removed or restyled in real time.' }));
  } else {
  ...
  return h('div', { class: 'view' },
    controls,
    h('p', { class: 'hint', text: '“Break on next mutation” freezes the page the instant a node is added — perfect for catching a dropdown’s markup as it opens.' }),
    feed,
  );
}
